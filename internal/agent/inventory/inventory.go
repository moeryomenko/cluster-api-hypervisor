...
func (s *Store) RecordOperation(installationID, ownerUID, key string, generation uint64) error {
	result, err := s.db.Exec(`INSERT OR IGNORE INTO operations(installation_id, owner_uid, idempotency_key, generation, created_at) VALUES (?, ?, ?, ?, ?)`, installationID, ownerUID, key, generation, time.Now().Unix())
	if err != nil {
...
func (s *Store) UpsertVM(vm VM) error {
	_, err := s.db.Exec(`INSERT INTO vms(installation_id, owner_uid, node_id, unit, generation, pid, disk, api_socket, vhost_socket, network, port, mac, ip, updated_at)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
ON CONFLICT(installation_id, owner_uid) DO UPDATE SET node_id=excluded.node_id, unit=excluded.unit, generation=excluded.generation, pid=excluded.pid, disk=excluded.disk, api_socket=excluded.api_socket, vhost_socket=excluded.vhost_socket, network=excluded.network, port=excluded.port, mac=excluded.mac, ip=excluded.ip, updated_at=excluded.updated_at`,
		vm.InstallationID, vm.OwnerUID, vm.NodeID, vm.Unit, vm.Generation, vm.PID, vm.Disk, vm.APISocket, vm.VhostSocket, vm.Network, vm.Port, vm.MAC, vm.IP, time.Now().Unix())
	if err != nil {
...
func (s *Store) GetVM(installationID, ownerUID string) (VM, error) {
	var vm VM
	err := s.db.QueryRow(`SELECT installation_id, owner_uid, node_id, unit, generation, pid, disk, api_socket, vhost_socket, network, port, mac, ip FROM vms WHERE installation_id=? AND owner_uid=?`, installationID, ownerUID).Scan(
