...
func TestHypervisorConfigValidateCreate(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "role with surrounding whitespace is rejected",
			give:    withRole(validConfig(), " control-plane "),
			wantErr: true,
		},
...
func TestHypervisorConfigValidateUpdate(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "valid to empty clusterName",
			oldObj:  validConfig(),
			newObj:  withClusterName(validConfig(), ""),
			wantErr: true,
		},
		{name: "invalid old can be fixed", oldObj: withRole(validConfig(), "master"), newObj: validConfig(), wantErr: false},
		{
			name:    "invalid to invalid",
			oldObj:  withRole(validConfig(), "master"),
			newObj:  withNodeName(validConfig(), ""),
			wantErr: true,
		},
		{
			name:    "wrong new object type",
			oldObj:  validConfig(),
			newObj:  &bootstrapv1alpha1.HypervisorConfigList{},
			wantErr: true,
		},
		{
			name:    "wrong old object type",
			oldObj:  &bootstrapv1alpha1.HypervisorConfigList{},
			newObj:  validConfig(),
			wantErr: true,
		},
