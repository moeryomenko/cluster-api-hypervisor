...
| Script | Role |
|---|---|
| `test/e2e/run.sh` | Full-lab orchestration: management plane up (or external), provider quadlet, ClusterClass + example Cluster, wait for workload Machines Ready, workload smoke checks, teardown via a trap. |
...
| `test/e2e/mgmt/` | Self-bootstrapped management plane: `pki.sh` (PKI + kubeconfigs), `apply.sh` (core manifests + quadlets), `down.sh` (stop the plane), `units/`, `core/`. |
...
2. **podman with quadlet support.** The management plane runs as quadlet
   ...
   (`test/e2e/mgmt/apply.sh:26`, `apply.sh:94`), and `make image` builds the
   provider image with podman (`Makefile:76-78`). `podman` and `systemctl` are
   required tools of the bootstrap (`test/e2e/mgmt/apply.sh:76-78`).
...
3. **The provider image.** Build it before the lab run so the provider quadlet
   ...
   `Makefile:77-78`). The image is local-only (never published); a quadlet
...
4. **The k8labs base image and firmware.** The provider boots workload VMs from
   ...
   (`docs/install-contract.md:98-99`). `run.sh` requires both as existing,
   readable, regular files (`run.sh:207-213`, `run.sh:216-222`); the relative
   defaults resolve against the working directory the harness is invoked from
   (`run.sh:32-38`, `run.sh:168-175`). Bake the base image with the k8labs
...
5. **cloud-hypervisor.** The binary is bundled inside the provider image at a
   ...
   (`docs/install-contract.md:103`). No separate host install is required; the
   host side is the `/dev/kvm` device.

6. **CLI tools.** `kubectl` is required by every scenario script
   (`run.sh:394`, `scale.sh:77-78`, `delete-cluster.sh:80-81`, `smoke.sh:35-36`).
   `run.sh` and `scale.sh` also need `base64` and `mktemp` (`run.sh:395-396`,
   `scale.sh:79-82`). The mgmt bootstrap needs `openssl` for `pki.sh` and,
   optionally, `kubectl` to render kubeconfigs (`test/e2e/mgmt/pki.sh:161-184`).
...
|---|---|---|---|
| `MANAGEMENT_KUBECONFIG` | unset -> mgmt-bootstrap fallback | Management-cluster kubeconfig. Set: must name an existing, readable, non-empty file; the plane is treated as external and is not torn down on exit. Unset/empty: the harness falls back to the committed bootstrap (`test/e2e/mgmt`) driven by `MGMT_STATE_DIR`; the state must be provisioned with the admin kubeconfig at `<state>/kubeconfigs/admin.conf`. | doc `run.sh:15-24`; validation `run.sh:181-197`; fallback `run.sh:188-193` |
| `MGMT_STATE_DIR` | `/var/lib/k8slab/mgmt` | Management-plane state directory for the fallback bootstrap (created by `test/e2e/mgmt/pki.sh`). | default `run.sh:72`; applied `run.sh:189`; also `test/e2e/mgmt/apply.sh:61` |
| `IMAGE` | `cluster-api-hypervisor:dev` | Provider image reference (the Makefile tag). A set value must be a syntactically plausible container reference (no whitespace). | default `run.sh:68`; applied/checked `run.sh:201-204` |
| `BASE_IMAGE` | `build/k8labs-base.qcow2` | k8labs base image path, resolved against the invocation working directory. Must be an existing, readable, regular file. | default `run.sh:69`; applied `run.sh:207`; checks `run.sh:208-213` |
| `FIRMWARE` | `build/CLOUDHV.fd` | CLOUDHV.fd path, resolved against the invocation working directory. Must be an existing, readable, regular file. | default `run.sh:70`; applied `run.sh:216`; checks `run.sh:217-222` |
| `STATE_DIR` | `/var/lib/k8slab` | Provider state directory. Must be an existing, writable directory (not a regular file). | default `run.sh:71`; applied `run.sh:225`; checks `run.sh:226-234` |
| `SMOKE` | `1` | Run the workload smoke checks (`smoke.sh`) after the Machines are Ready. `0` disables them; any other value enables them. When `smoke.sh` is absent the checks are skipped with a note. | applied `run.sh:388`; disabled check `run.sh:347-349`; absent check `run.sh:351-355` |
| `WAIT_TIMEOUT` | `1800` | Seconds to wait for the workload Machines to become Ready. | default `run.sh:80`; applied `run.sh:389`; used `run.sh:306` |

Validation is all-or-nothing before any heavy work: no cluster, VM, or quadlet
is started unless every variable above is valid (`run.sh:11-12`, `run.sh:179`).
...
namespace `default` (`run.sh:76-77`), the worker MachineDeployment is `md-0`
...
1. **mgmt up** (`run.sh:398`, `mgmt_up` at `run.sh:251-261`). With
   ...
   core manifests from `test/e2e/mgmt/core/` and installs the quadlet units
   from `test/e2e/mgmt/units/` as `mgmt-etcd`,
   `mgmt-kube-apiserver`, `mgmt-cluster-api-core`, and
   `mgmt-cluster-api-hypervisor` services, then starts them (`apply.sh:83-119`).
2. **Provider** — wait for the provider quadlet `mgmt-cluster-api-hypervisor`
   to be active (`run.sh:400`, `wait_for_provider` at `run.sh:279-290`;
   fallback plane only, `run.sh:280`).
3. **ClusterClass + Cluster** — apply the committed topology to the management
   cluster (`run.sh:401`, `apply_templates` at `run.sh:294-301`):
   `templates/clusterclass.yaml` and `templates/cluster-example.yaml`
   (single control-plane replica + three worker replicas,
   `templates/README.md`).
4. **Machines Ready** — poll until every workload Machine reports the CAPI
   `Ready` condition `True` (`run.sh:402`, `wait_for_machines_ready` at
   `run.sh:305-329`).
5. **Smoke** — extract the workload kubeconfig from the
   `k8labs-kubeconfig` Secret (`run.sh:333-342`) and run the workload smoke
   checks (`run.sh:403`, `run_smoke` at `run.sh:346-360`), unless `SMOKE=0`.
6. **Teardown** — on exit, the trap (`run.sh:365-384`) deletes the workload
...
`run.sh` requires `kubectl`, `base64`, and `mktemp` on PATH (`run.sh:394-396`).
Exit codes: `0` full-lab run completed (including teardown) or `--help`;
`1` environment validation failure or orchestration failure (`run.sh:49-52`).
`test/e2e/run.sh --help` prints the full contract without touching anything.

### Teardown semantics (read before Phase 2/3)

`run.sh` always deletes the workload Cluster on exit — with an external
management plane the Cluster is still deleted (`run.sh:368-374`), only the
plane itself is left running (`run.sh:378-382`). So Phases 2 and 3 run against
...
KC=/var/lib/k8slab/mgmt/kubeconfigs/admin.conf   # the fallback admin kubeconfig (run.sh:190)
kubectl --kubeconfig="$KC" apply -f templates/clusterclass.yaml -f templates/cluster-example.yaml
...
`apply.sh` is idempotent (`apply.sh:14-16`), so re-running it converges; the
templates apply is declarative (`run.sh:292-293`). After Phase 3 the lab is
down and the host network state is verified gone.
...
|---|---|---|---|---|
| run.sh | management apiserver `/readyz` | 300 s | 2 s | `run.sh:79`, `run.sh:265-275` |
| run.sh | provider quadlet `mgmt-cluster-api-hypervisor` active | 300 s | 2 s | `run.sh:281-289` |
| run.sh | workload Machines Ready | `WAIT_TIMEOUT`, default 1800 s | 5 s | `run.sh:80`, `run.sh:82`, `run.sh:305-329` |
| run.sh | teardown: workload Cluster deletion | 300 s | — | `run.sh:81`, `run.sh:370-373` |
...
existing, readable, non-empty kubeconfig (`run.sh:181-187`); the mgmt
bootstrap is then skipped entirely (`run.sh:252-255`) and the plane is left
running on exit (`run.sh:378-382`). This is the supported way to re-run the
...
With an external plane, `run.sh` still applies the ClusterClass + example
Cluster, waits for Ready, runs smoke, and still deletes the workload Cluster on
exit — only the plane itself survives (`run.sh:368-382`). If the external
plane lacks the CAPI core CRDs, the provider, or the host network setup the
...
|---|---|---|
| `test/e2e/mgmt/mgmt_test.sh` | `pki.sh` state layout, quadlet units, core manifests, `apply.sh`/`down.sh` lifecycle | Executes `pki.sh` against a scratch state directory; every other assertion checks the committed files directly (`mgmt_test.sh:48-50`). |
| `test/e2e/harness_test.sh` | `run.sh` validates every contract variable before heavy work and its errors name the exact variable | Runs `run.sh` with `env -i` (only `PATH`/`HOME` plus explicit assignments) and a 30 s timeout, feeding fake-but-valid fixture files; every scenario keeps all variables valid except the one under test (`harness_test.sh:114-137`, `harness_test.sh:186-197`). Never starts a cluster, VM, or quadlet (`harness_test.sh:55-60`). |
...
- **`run.sh` exits 1 before any heavy work**: the error names the offending
  variable and path (`run.sh:179-234`); check the env table above against the
  invocation.
...
- **The provider quadlet does not start**: check
  ...
  (`run.sh:283-285`); confirm the provider image exists
